...
from application.custom_user_validator import validate, check_order
...
def employee_view_employee_leaves(request, id):
    ...
    if user:
        ...
        if request.method == 'GET':    
            ...
            _status = request.POST.get('statusList',False)
            
            _notedBy, _checkedBy, _approvedBy = leave.noted_by, leave.checked_by, leave.approved_by
 
            current_user_role = user.company_to_user.employee_ci_rp_fk_r.role
            if current_user_role == 'Human Resource': 
                _checkedBy = str(user.profile_to_user)  
            elif current_user_role == 'Managing Director':
                _approvedBy = str(user.profile_to_user)
...
def employee_view_itenerary(request, id):
    ...
    if user:
        ...
        if request.method == 'GET':
            ...
            _notedBy, _checkedBy, _approvedBy = itenerary.noted_by, itenerary.checked_by, itenerary.approved_by
...
def employee_view_overtime(request, id):
    ...
    if user: 
        ...
        if request.method == 'GET':
            ...
            _notedBy, _checkedBy, _approvedBy = overtime.noted_by, overtime.checked_by, overtime.approved_by
...
def employee_create_roles_permission(request):
    ...
    if user:
        ...
        if request.method == 'GET':
            ...
            im_emp = PersonalInfo.objects.annotate(name=Concat('last_name', Value(' '),'first_name', Value(' '),'middle_name'),).get(Q(name__icontains=_immidiate_supervisor))
 
            new, existing = RolesPermission.objects.update_or_create(employee_ci_rp_fk=c_emp, defaults={
                'role':_role,
                'title':_title,
                'immidiate_head':im_emp.fk_user.company_to_user
            })
...
def employee_edit_roles_permission(request, id):
    ...
    if user: 
        ...
        if request.method == 'GET':
            ...
            _immidiate_head = roles_permission.immidiate_head.fk_company_user.profile_to_user if roles_permission.immidiate_head != None else "-" 
...
def form_manager(request):
    ...
    if user:
        if request.method == 'GET':
            ...
            #     elif hasattr(item,'employee_itenerary_fk'):
            #         print("ITINERARY")
            #         print(item.employee_itenerary_fk)
            #     elif hasattr(item,'employee_overtime'):
            #         print("OT")
            #         print(item.employee_overtime)
            #     else:
            #         continue
            context = {
                'user': user,   
...
def side_employee_view_leave_form(request, id):
    ...
    if user:
        ...
        notifications_count = notifications.count()
        if request.method == 'GET': 
            current_user = user.company_to_user.employee_ci_rp_fk_r.employee_ci_rp_fk
            from_emp_im_head = leave.employee_leave_fk.fk_user.company_to_user.employee_ci_rp_fk_r.immidiate_head
            is_approvable = True if current_user == from_emp_im_head else False 
        elif request.method == 'POST': 
            current_user_role = user.company_to_user.employee_ci_rp_fk_r.role
            if current_user_role == 'Business Unit Head': 
                current_user = user.company_to_user.employee_ci_rp_fk_r.employee_ci_rp_fk
                from_emp_im_head = leave.employee_leave_fk.fk_user.company_to_user.employee_ci_rp_fk_r.immidiate_head
                _notedBy = str(user.profile_to_user)   
                if current_user == from_emp_im_head: 
                    new, existing = EmployeeLeaves.objects.update_or_create(id=id, defaults={ 
                        'noted_by': _notedBy,  
                    }) 
            
            return HttpResponseRedirect(reverse_lazy('application:employee_side_form_manager'))
...
def side_employee_view_itenerary_form(request, id):
    ...
    if user:  
        ...
        if request.method == 'GET':
            current_user = user.company_to_user.employee_ci_rp_fk_r.employee_ci_rp_fk
            from_emp_im_head = itenerary.employee_itenerary_fk.fk_user.company_to_user.employee_ci_rp_fk_r.immidiate_head
            is_approvable = True if current_user == from_emp_im_head else False 
        elif request.method == 'POST':
            current_user_role = user.company_to_user.employee_ci_rp_fk_r.role
            if current_user_role == 'Business Unit Head': 
...
def side_employee_view_overtime_form(request, id):
    ...
    if user:
        ...
        if request.method == 'GET':
            current_user = user.company_to_user.employee_ci_rp_fk_r.employee_ci_rp_fk
            from_emp_im_head = overtime.employee_overtime.fk_user.company_to_user.employee_ci_rp_fk_r.immidiate_head
            is_approvable = True if current_user == from_emp_im_head else False 
        elif request.method == 'POST':
            current_user_role = user.company_to_user.employee_ci_rp_fk_r.role
            if current_user_role == 'Business Unit Head': 
...
def employee_side_form_manager(request):
    ...
    user = get_object_or_404(User, username=request.user.username) 
    user = validate(user, is_business_unit_head=True) 
    if user:
        if request.method == 'GET':
            notifications = Notifications.objects.all().filter(Q(Q(recipient=user) | Q(public=True)) & Q(is_read=False)).order_by('-id')
            notifications_count = notifications.count()
